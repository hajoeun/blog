...
## 1) 폴더/파일 구조
- **위치**: `src/posts/{year}/{id}.{md,mdx}`
- **제한**: 연도 폴더 바로 아래 1단계까지만 인식해요. 그보다 깊은 서브폴더는 무시돼요.
- **확장자**: `.md`와 `.mdx`를 게시글로 처리해요.
- **선택 기준**: 순수 Markdown/GFM만 쓰면 `.md`, `<Caption>` 같은 React 컴포넌트나 JSX가 필요하면 `.mdx`를 사용하세요.
- **중복 금지**: 같은 `{year}/{id}`에 `.md`와 `.mdx`를 동시에 만들면 빌드가 실패해요.

예)
- `src/posts/2025/0929.mdx`
- `src/posts/2025/personal-market-fit.md`
...
여기부터 본문을 작성합니다. Markdown/MDX 문법과 GFM(표, 체크리스트 등)을 사용할 수 있어요.
```
...
## 4) 본문 작성 규칙(Markdown/MDX)
- **지원**: 기본 Markdown/MDX, GFM(테이블, 체크박스, 자동 링크 등)
- **`.md` 권장 상황**: 일반 본문, 링크, 이미지, 코드블록, GFM만 필요한 글
- **`.mdx` 권장 상황**: React 컴포넌트, JSX 속성, JavaScript expression이 필요한 글
...
## 6) 새 글 추가 절차(체크리스트)
1. 파일 생성: `src/posts/{year}/{id}.md` 또는 `src/posts/{year}/{id}.mdx`
2. 프론트매터 채우기: 위 스키마 그대로 복사 → 값만 수정
3. 본문 작성: Markdown/MDX/GFM 사용, H2부터 시작 권장
4. 로컬 확인:
...
- 잘못된 폴더 깊이 → `src/posts/{year}/{id}.{md,mdx}`만 인식 (추가 하위 폴더 불가)
- 확장자 누락 → `.md` 또는 `.mdx`만 게시글로 처리
- 중복 slug → 같은 경로에 `{id}.md`와 `{id}.mdx`를 함께 만들지 않기
...
Notion에 작성해 둔 초안이 있다면 자동으로 가져와서 Markdown 또는 MDX 파일로 변환할 수 있어요.
...
### 워크플로우
1. **초안 파일 생성**: `src/posts/{year}/{id}.md` 또는 `src/posts/{year}/{id}.mdx` 템플릿 파일 생성
2. **Notion 조회**: 
   - 제목으로 검색하거나
   - Notion 링크를 직접 사용
3. **내용 가져오기**: Notion 페이지 전체 내용을 fetch
4. **Markdown/MDX 변환**: 
   - Notion의 텍스트 내용을 Markdown 본문으로 변환
   - React 컴포넌트나 JSX가 필요하면 MDX 본문으로 조정
...
### 주의사항
- Notion의 서식은 자동 변환되지 않으므로 Markdown/MDX 문법에 맞게 수동 조정 필요
...
에이전트가 자동으로:
1. 오늘 날짜(예: 2025년 10월 7일 → `1007.md`)로 파일 생성
2. Notion 페이지 내용 가져오기
3. Markdown/MDX 형식으로 변환 (제목, description, 본문)
4. 더 나은 제목이 있다면 제안
5. 커밋까지 완료
