...
**자바스크립트에서 객체는 키(key)와 값(value)의 쌍으로 이루어져있습니다.** 프로그래밍을 하는 과정에서 키를 기준으로 값을 객체로부터 꺼내는 일을 반복합니다. 많은 데이터를 가진 객체 하나에서 여러 개의 값을 꺼내오려면 어떻게 할까요?
...
$.post('/api/user_name/check', user_data).done(redirect('/main')); // [3] '오늘의 함수 redirect' 편을 참고하세요.
```
...
$.post('/api/city/check', pick(user_1, ['id', 'city'])).done(redirect('/main'));
```
